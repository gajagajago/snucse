...
//======================================================Implement below=================================================================//
	...
	void markPawn(int x, int y, PlayerColor pc) {
		...
		switch (y) {
			case 0:
				if(isEnemy(x+move, y+1, pc)) { possibleMoves.add(new Point(x+move, y+1)); }
				break;
			case 7:
				if(isEnemy(x+move, y-1, pc)) { possibleMoves.add(new Point(x+move, y-1)); }
				break;
	...
	void markRook(int x, int y, PlayerColor pc) {
		//mark up
		for(int i = x-1; i >= 0; --i) {
			if(getIcon(i,y).type == PieceType.none) possibleMoves.add(new Point(i,y));
		...
		//mark left
		for(int i = y-1; i >= 0; --i) {
			if(getIcon(x,i).type == PieceType.none) possibleMoves.add(new Point(x,i));
	...
	void markKnight(int x, int y, PlayerColor pc) {
		for(int i = x-2; i <= x+2; i+=4) {
			for(int j = y-1; j <= y+1; j +=2) {
				if((i>=0) && (i<8) && (j>=0) && (j<8)) {
					if(!isAlly(i,j,pc)) possibleMoves.add(new Point(i,j));
				}
			}
		}
	...
	void markBishop(int x, int y, PlayerColor pc) {
		...
		for(int i = x-1, j = y-1; i >= 0 && j >= 0; --i, --j) {
			if(getIcon(i,j).type == PieceType.none) possibleMoves.add(new Point(i,j));
			if(isAlly(i,j,pc)) break;
			if(isEnemy(i,j,pc)) {
	...
	Point findKing(PlayerColor pc) {
		for(int i = 0; i < 8; ++i) {
			for(int j = 0; j < 8; ++j) {
				if(isAlly(i,j,pc) && getIcon(i,j).type == PieceType.king) return new Point(i,j);
			}
		}
